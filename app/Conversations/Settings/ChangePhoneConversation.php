...
class ChangePhoneConversation extends BaseConversation
{
    ...
    {
        $question = ComplexQuestion::createWithSimpleButtons(
            $message ?: Translator::trans('messages.enter sms code', ['phone' => $this->getUser()->phone]), ['call', 'back']
        );
        ...
        return $this->ask($question, function (Answer $answer) {
            if ($answer->getValue() == 'call') {
              ...
              $this->run();
                ...
                $this->confirmSms(Translator::trans($this->getFromStorage('called') ? 'messages.incorrect phone code' : 'messages.wrong sms code'));
            }
        });
    }
}