...
class OrderApiService
{
	...
	public function createOrder(BotMan $bot, $useBonus = false)
	{
        ...
        $params =[
            ...
            'content' => json_encode([
                ...
                'is_prior' => false,
                'order_params' => $options->getOrderParamsArray($bot->userStorage())->toArray(),
                'phone' => $phone,
        ...
        $response = $this->file_get_contents_with_logging('https://sk-taxi.ru/tmapi/api.php?method=%2Fcommon_api%2F1.0%2Fcreate_order2',$params);
    ...
    public function getCurrentOrders(User $user)
    {
        ...
        $response = $this->file_get_contents_with_logging('https://sk-taxi.ru/tmapi/api.php?method=%2Fcommon_api%2F1.0%2Fget_current_orders',$params);
	...
	public function getOrderState($order)
	{
		$params = [
		 ...
		 $response = $this->file_get_contents_with_logging('https://sk-taxi.ru/tmapi/api.php?method=%2Fcommon_api%2F1.0%2Fget_order_state',$params);
	...
	public function changeOrderState($order, $state)
	{
        ...
        $response = $this->file_get_contents_with_logging('https://sk-taxi.ru/tmapi/api.php?method=%2Fcommon_api%2F1.0%2Fchange_order_state',$params);
	 ...
	 public function changeOrderPrice(BotMan $bot,$order)
    {
        ...
        $response = $this->file_get_contents_with_logging('https://sk-taxi.ru/tmapi/api.php?method=%2Fcommon_api%2F1.0%2Fupdate_order',$params);
    ...
    public function connectClientAndDriver($order)
	{
		$params = [
		  ...
		  $response = $this->file_get_contents_with_logging('https://sk-taxi.ru/tmapi/api.php?method=%2Ftm_tapi%2F1.0%2Fconnect_client_and_driver',$params);
	...
	public function selectTariffForOrder($crewGroupId, $lat, $lon)
	{
       ...
       foreach($lat as $key => $item){
        ...
        $response = $this->file_get_contents_with_logging('https://sk-taxi.ru/tmapi/api.php?method=%2Fcommon_api%2F1.0%2Fselect_tariff_for_order',$params);
	...
	public function calcOrderPrice($tariffId, $orderParams, Storage $storage)
	{
	    ...
	    $response = $this->file_get_contents_with_logging('https://sk-taxi.ru/tmapi/api.php?method=%2Fcommon_api%2F1.0%2Fcalc_order_cost2',$params);
        return json_decode($response);
    }
    ...
    public function analyzeRoute(Storage $storage)
    {
        ...
        $response = $this->file_get_contents_with_logging('https://sk-taxi.ru/tmapi/api.php?method=/common_api/1.0/analyze_route2',$params);
    ...
    public function sendSMSCode($phone, $code)
    {
        ...
        return file_get_contents(
            'https://sk-taxi.ru/tmapi/sms.php?key='.env('SMS_KEY', 'FN47XhzkAhS4jnbdyt5D').'&to='.$phone.'&message='.urlencode(env('SMS_TEXT', 'Ваш код подтверждения:').' '.$code),
            false
        );
    }
    public function callSMSCode($phone, $code)
    {
        ...
        return file_get_contents(
            'https://sk-taxi.ru/tmapi/call.php?key='.env('SMS_KEY', 'FN47XhzkAhS4jnbdyt5D').  '&to=' . $phone . '&message=' .$code,
            false
        );
    }
    ...
    public function driverTimeCount($orderId)
    {
        ...
        $response = $this->file_get_contents_with_logging('https://sk-taxi.ru/tmapi/api.php?method=%2Ftm_tapi%2F1.0%2Fget_info_by_order_id',$params);
        return json_decode($response);
    }
